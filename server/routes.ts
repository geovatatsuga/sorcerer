...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Translation proxy endpoint - supports batching. Requires TRANSLATE_PROVIDER and TRANSLATE_API_KEY env vars.
  app.post('/api/translate', async (req, res) => {
    try {
      const provider = process.env.TRANSLATE_PROVIDER;
      const key = process.env.TRANSLATE_API_KEY;
      if (!provider || !key) return res.status(501).json({ message: 'Translation provider not configured' });

      const { texts, target, source } = req.body as { texts?: string[]; target?: string; source?: string };
      if (!texts || !Array.isArray(texts) || !target) return res.status(400).json({ message: 'texts (string[]) and target are required' });

      if (provider === 'deepl') {
        // DeepL expects target_lang e.g. 'PT' or 'EN'
        const params = new URLSearchParams();
        params.append('auth_key', key);
        for (const t of texts) params.append('text', t);
        params.append('target_lang', (target || 'EN').toUpperCase());
        if (source) params.append('source_lang', source.toUpperCase());
        const resp = await fetch('https://api-free.deepl.com/v2/translate', { method: 'POST', body: params });
        const json = await resp.json();
        const translations = (json.translations || []).map((x: any) => x.text);
        return res.json({ translations });
      }

      if (provider === 'google') {
        const url = `https://translation.googleapis.com/language/translate/v2?key=${encodeURIComponent(key)}`;
        const body: any = { q: texts, target };
      ...
      return res.status(501).json({ message: 'Unsupported translation provider' });
    } catch (error) {
  ...
  if (process.env.NODE_ENV === 'development' && !process.env.REPLIT_DOMAINS) {
    ...
    // Translation helpers: store per-resource translations in data/translations/{resource}.{id}.json
    app.get('/api/dev/translations/:resource/:id', async (req, res) => {
      try {
        ...
        const fp = path.join(translationsDir, `${resource}.${id}.json`);
        try {
          const txt = await fs.promises.readFile(fp, 'utf-8');
          const obj = JSON.parse(txt || '{}');
          return res.json({ ok: true, translation: obj[lang] ?? null });
        } catch (e) {
          return res.json({ ok: true, translation: null });
        }
      } catch (error) {
        console.error('Dev get translation error:', error);
        return res.status(500).json({ message: 'Failed to read translation' });
      }
    });
    ...
    app.post('/api/dev/translations/:resource/:id', async (req, res) => {
      try {
        const { resource, id } = req.params;
        const { lang, data: payload } = req.body || {};
        if (!lang || !payload) return res.status(400).json({ message: 'lang and data required' });
        const translationsDir = path.resolve(process.cwd(), 'data', 'translations');
        await fs.promises.mkdir(translationsDir, { recursive: true });
        const fp = path.join(translationsDir, `${resource}.${id}.json`);
        let obj: any = {};
        try { const existing = await fs.promises.readFile(fp, 'utf-8'); obj = JSON.parse(existing || '{}'); } catch (e) { obj = {}; }
        obj[lang] = payload;
        await fs.promises.writeFile(fp, JSON.stringify(obj, null, 2), 'utf-8');
        return res.json({ ok: true });
      } catch (error) {
        console.error('Dev save translation error:', error);
